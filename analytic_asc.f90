! Copyright (c) 2017 - 2019, Philip Herd
! This file is distributed under the BSD 2-Clause License

module analytic  ! ASCENDING ORDER!
    implicit none
    ...
    ! this file accepts the coefficients in ascending order: cof(n) /= 0
    !    (cof(1) * x^0 + cof(2) * x^1 + cof(3) * x^2 + ...)
    ! for coefficients in descending order, see 'analytic_dsc.f90'
    ...
    contains
        ...
        subroutine root_1(cof, rts)  ! linear,    b + ax = 0
            complex(dp), intent(in)  :: cof(:)
            complex(dp), intent(out) :: rts(:)
            rts(1) = -cof(2) / cof(1)
        end subroutine root_1

        subroutine root_2(cof, rts)  ! quadratic, c + bx + ax^2 = 0
            complex(dp), intent(in)  :: cof(:)
            complex(dp), intent(out) :: rts(:)
            complex(dp) :: p, q
        ...
        subroutine root_3(cof, rts)  ! cubic,     d + cx + bx^2 + ax^3 = 0
            complex(dp) :: abc, bbb, aad, dd, d0, d1, cin, cc, p, x1
            complex(dp), intent(in)  :: cof(:)
            complex(dp), intent(out) :: rts(:)

            abc = cof(4) * cof(3) * cof(2)
            bbb = cof(3) * cof(3) * cof(3)
            aad = cof(4) * cof(4) * cof(1)

            dd = 18.0_dp * abc * cof(1) - 4.0_dp * bbb * cof(1)
            dd = dd + cof(3) * cof(3) * cof(2) * cof(2) - 4.0_dp * cof(4) * cof(2) * cof(2) * cof(2)
            dd = dd - 27.0_dp * aad * cof(1)
            d0 = cof(3) * cof(3) - 3.0_dp * cof(4) * cof(2)

            if ((dd == 0).and.(d0 == 0)) then  ! triple root
                x1 = -cof(3) / (3.0_dp * cof(4))
                rts(1) = x1
                rts(2) = x1
                rts(3) = x1
            else if ((dd == 0).and.(d0 /= 0)) then  ! double root, simple root
                x1 = ((9.0_dp * cof(4) * cof(1) - cof(3) * cof(2)) / (2.0_dp * d0))
                rts(1) = x1
                rts(2) = x1
                rts(3) = (4.0_dp * abc - 9.0_dp * aad - bbb) / (cof(4) * d0)
            else
                ...
                if (d0 == 0) then
                    ...
                    cin = (d1 - sqrt(-27.0_dp * cof(4) * cof(4) * dd)) / 2.0_dp
                end if
                cc = cin ** (1.0_dp / 3.0_dp)
                p = (-1.0_dp / (3.0_dp * cof(4)))

                rts(1) = p * (cof(3) + cc + d0 / cc)
                rts(2) = p * (cof(3) + cu2 * cc + d0 / (cu2 * cc))
                rts(3) = p * (cof(3) + cu3 * cc + d0 / (cu3 * cc))
            end if
        end subroutine root_3

        subroutine root_4(cof, rts)  ! quartic,   e + dx + cx^2 + bx^3 + ax^4 = 0
